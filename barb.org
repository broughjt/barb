...
I'm assuming this file will become much bigger if this project lasts
...
The definition of a commutative ring.
...
This is totally above my paygrade right now. I don't even know what
the Relator is, but it looks important. I will return to this at a
future time. For now, I'm copy-pasting it so that I have a fighting
chance of using quotient types for the integers and rationals.
...
Copy-paste documentation from mathlib4:

The binary relations ~R : α → β → Prop~ and ~S : γ → δ → Prop~ induce
a binary relation on functions ~LiftFunction : (α → γ) → (β → δ) → Prop~.
...
The syntax ~(R ⇒ S) f g~ means the same thing as ~LiftFunction R S f g~.
...
Just some syntax for 0 and 1 literals.
...
This section continues. It feels pretty disorganized and needs
revision.

Now we talk about decidability for the order relations. First we
define ~booleanLessEqual~.
...
The Natural numbers aren't closed under subtraction, so we define an
integer to basically /be/ this as of yet missing notion of subtraction
of two natural numbers. We do this by constructing the integers as a
quotient type on $\mathbb{N} \times \mathbb{N}$ with respect to the
relation $n + m' = n' + m$ where $(n, m)$ and $(n', m')$ are
integers. We do this because we haven't yet defined subtraction, but
we know that it should obey that algebraic law.
...
We show that ~integerEquivalent~ is an equivalence relation, and then
use it in the quotient construction of the integers.

I can't seem to get the simplifier to work for the proof of
transitivity, so I'm left with a really ugly ~calc~ proof. Need to
revisit the simplifier and get it to work here.
...
The integers are the set of equivalence classes of expressions of the form $n - m$ for
some $n, m \in \mathbb{N}$. Here $n - m$ is just symbolic notation
for a pair of natural numbers, since we haven't yet defined
subtraction.
...
To negate $n - m$, simply return $m - n$.

TODO: Write more about the ~Quotient.map~ operation.
...
The add two integers, we add the natural numbers on their left and
right hand sides.

TODO: The simplifier can't find the ~calc~ proof below. Figure out how
to get it to.
...
For the additive properties of communitive rings, simply apply the
induction princple on quotients and ~Quotient.sound~, and then appeal
the properties we developed the natural numbers.
...
We define multiplication in the following way. Suppose we have
$(n - m)(k - l)$. Distributing gives $nk - nl - mk + ml$, and moving
the positive terms to the left, the negative terms to the right, and
factoring the sign outside of the right hand side parentheses, we have
$(nk + ml) - (nl + mk)$, which we use for our definition of
